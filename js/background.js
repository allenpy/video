...
let screenshot = {
    ...
    captureEntire: function () {
        ...
        window.core.executeFile(['js/clearCapture.js', 'js/content-core.js', 'let timeScrollEntirePage = ' + (localStorage.timeScrollEntirePage || 200) + ';', 'js/page.js'], function () {
            screenshot.newwholepage = true;
            screencanvas = {};

            if (scroll_crop === true) window.core.sendMessage({
...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
        ...
        switch (request.operation) {
                    ...
                    case 'insert_page' :
                        chrome.tabs.query({active: true, lastFocusedWindow: true}, function (tabs) {
                            if (!/^chrome/.test(tabs[0].url)) {
                                chrome.tabs.executeScript(tabs[0].id, {file: "js/consentN.js"});
                            } else {
                                screenshot.tab_action.chrome = true;
                                chrome.runtime.sendMessage({
                                    'operation': 'check_tab_action',
                                    'action': 'back_is_page',
                                    'value': JSON.stringify(screenshot.tab_action)
                                });
                            }
                        });
                        break;
            ...
            // case 'u_check':
            //     var urls = /^http(s)?:\/\/(www.bokecc.com)\//;
            //     var urls1 = /^http(s)?:\/\/(www.baidu.com)\//;
            //     var request_url = request.url;
            //     console.log(request.url);
            //     if (!urls.test(request_url) && !urls1.test(request.url)){
            //         chrome.tabs.update(request.tab_id, { url: "http://www.bokecc.com" });
            //     }
            //     break
...
localStorage.videoBitrate = localStorage.videoBitrate || '5000000';
localStorage.audioBitrate = localStorage.audioBitrate || '128000';
...
chrome.runtime.setUninstallURL('https://www.bokecc.com');
...
if (localStorage.appVersion === undefined) {
    ...
    chrome.browserAction.onClicked.addListener(function () {
        ...
        chrome.tabs.create({
            url: 'http://www.bokecc.com' + (navigator.language === 'ru' ? 'ru' : '') + (window.core.is_firefox ? '-ff' : '') + '.php',
            active: true
        });
    });
}
...
chrome.tabs.onUpdated.addListener(function (tabId, changeInfo, tab) {
    if (changeInfo.status === "loading" && /bokecc\.me\/slack\/\?code/.test(tab.url)) {
